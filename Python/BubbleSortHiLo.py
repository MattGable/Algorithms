'''
NOTE: This is a "high-to-low" bubble sort. 
This bubble sort will iterate over n-1 elements, with the smallest value being
"bubbled" to the end(the right in this case) on each pass. This will loop
roughly i*j times (n*n or n^2 time complexity). On each pass (j), the smallest
...
NOTE: This is a "backwards" version, which reversed the usual low-to-high
(left-to-right) ordering--for fun. For the standard order, please see:
"BubbleSortLoHi.py" in this folder. As with the insertion sort, simply changing
whether to swap high or low values during a comparison changes the ordering
of the sort. The only change was swapping a "less than" for a "greater than."
'''
